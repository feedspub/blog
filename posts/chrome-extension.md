---
title: Feeds Pub chrome extension, badge and more updates
date: 2020-05-06
---

## Chrome Extension

Feeds Pub got a [chrome extension](https://chrome.google.com/webstore/detail/feeds-pub/pallehogfdjngkmogmijfllooflflaof)🎉. The functionality is pretty simple, the extension shows you how many unread contents you have got on Feeds Pub so you don't need to check it manually.

(It checks updates every 10 minutes)

### How to install

- [Chrome Web Store](https://chrome.google.com/webstore/detail/feeds-pub/pallehogfdjngkmogmijfllooflflaof)
- [GitHub](https://github.com/FeedsPub/extension/releases)

### Screenshot

![](https://timqian-imgs.s3.ap-southeast-1.amazonaws.com/2020-05-Screen%20Shot%202020-04-27%20at%205.21.00%20PM.png)


## [Feeds Pub Badge](https://badge.feeds.pub)

A Feeds Pub follower counter which can be embed to your blog.

After adding a badge, please tell me on [GitHub](https://github.com/FeedsPub/badge/issues), so that I can add your link on badge.feeds.pub

#### Examples:

[![](https://img.shields.io/badge/dynamic/json?label=follow&query=%24.data.feed.followerCount&url=https%3A%2F%2Fapi.feeds.pub%2Fgraphql%3Fquery%3Dquery%2520feed%28%2524id%253A%2520String%21%29%257B%2520feed%28id%253A%2520%2524id%29%2520%257B%2520followerCount%2520%257D%2520%257D%26variables%3D%257B%2522id%2522%253A%2520%2522https%3A%2F%2Fblog.t9t.io%2Fatom.xml%2522%257D%26operationName%3Dfeed&style=social&logo=data:image/png;base64,)](https://feeds.pub/feed/https%3A%2F%2Fblog.t9t.io%2Fatom.xml)

[![](https://img.shields.io/badge/dynamic/json?label=follow&color=61B04B&query=%24.data.feed.followerCount&url=https%3A%2F%2Fapi.feeds.pub%2Fgraphql%3Fquery%3Dquery%2520feed%28%2524id%253A%2520String%21%29%257B%2520feed%28id%253A%2520%2524id%29%2520%257B%2520followerCount%2520%257D%2520%257D%26variables%3D%257B%2522id%2522%253A%2520%2522https%3A%2F%2Fblog.t9t.io%2Fatom.xml%2522%257D%26operationName%3Dfeed&style=flat&logo=data:image/png;base64,)](https://feeds.pub/feed/https%3A%2F%2Fblog.t9t.io%2Fatom.xml)

[![](https://img.shields.io/badge/dynamic/json?label=follow&color=61B04B&query=%24.data.feed.followerCount&url=https%3A%2F%2Fapi.feeds.pub%2Fgraphql%3Fquery%3Dquery%2520feed%28%2524id%253A%2520String%21%29%257B%2520feed%28id%253A%2520%2524id%29%2520%257B%2520followerCount%2520%257D%2520%257D%26variables%3D%257B%2522id%2522%253A%2520%2522https%3A%2F%2Fblog.t9t.io%2Fatom.xml%2522%257D%26operationName%3Dfeed&style=flat-square&logo=data:image/png;base64,)](https://feeds.pub/feed/https%3A%2F%2Fblog.t9t.io%2Fatom.xml)

### How to add

- Visit [badge.feeds.pub](https://badge.feeds.pub)

## Other updates

Mark one content as read
- Automatically mark content as read after user visit the link
- Manually mark one content as read

## Feedback?

If you have any suggestions or feedback, feel free to

- Open an issue on [GitHub](https://github.com/feedspub/feedspub-feedback/issues)
- Chat with us on [Telegram](https://t.me/feedspub_chat)
